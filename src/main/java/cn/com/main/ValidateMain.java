	...
	public static void validateByKM(List<Point> points) {
		...
		for(int i=1; i<=20; i++) {			
			...
			if(cluster != null && cluster.size() >= 1){
				...
				System.out.println("when the k is " + i + "  the number of Cluster is : " + cluster.size() + " the purity is :" + purity*100 + "%");
				System.out.println("when the k is " + i + "  the number of Cluster is : " + cluster.size() + " the F-score is :" + FScore*100 + "%");
			}
		}
